"""
Nearest Neighbor heuristic for the Travelling Salesman Problem (TSP).

Greedy algorithm: at each step, go to the nearest unvisited city.
Fast O(N²) but not optimal. Used as a baseline for comparison with Simulated Annealing.
"""

from app.algorithms.tsp_tw_utils import evaluate_route


def solve(
    matrix: list[list[float]],
    start_index: int = 0,
    time_windows: list[tuple[float, float]] | None = None,
    service_time: float = 0,
) -> tuple[list[int], float]:
    """
    Solve TSP using the Nearest Neighbor heuristic.

    Args:
        matrix: 2D list of travel times in seconds. matrix[i][j] = time from i to j.
        start_index: Which location to start from (default 0).
        time_windows: Optional list of (earliest, latest) per location. If provided,
            cost is penalized (travel_time + 10000 per violation). NN does not
            optimize for windows; it just reports the penalized cost.
        service_time: Seconds spent at each location (for TSPTW evaluation).

    Returns:
        (route, total_cost) where route is a list of indices in visit order
        (including return to start), and total_cost is travel time or penalized cost.

    Raises:
        ValueError: If matrix is empty, not square, or start_index is out of range.
    """
    n = len(matrix)
    if n == 0:
        return ([], 0.0)

    if start_index < 0 or start_index >= n:
        raise ValueError(f"start_index {start_index} out of range for matrix size {n}")

    for row in matrix:
        if len(row) != n:
            raise ValueError(f"Matrix must be square, got {n}x{len(row)}")

    visited = {start_index}
    route = [start_index]
    current = start_index
    total_cost = 0.0

    while len(visited) < n:
        best_j = -1
        best_time = float("inf")
        for j in range(n):
            if j not in visited:
                t = matrix[current][j]
                if t < best_time:
                    best_time = t
                    best_j = j

        route.append(best_j)
        total_cost += best_time
        current = best_j
        visited.add(best_j)

    # Return leg to start
    total_cost += matrix[current][start_index]
    route.append(start_index)

    if time_windows is not None:
        total_cost, _ = evaluate_route(
            route, matrix, time_windows, service_time=service_time
        )

    return (route, total_cost)
